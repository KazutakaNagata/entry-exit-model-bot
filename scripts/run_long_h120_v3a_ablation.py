#!/usr/bin/env python3
"""Build features and train long_H120 entry grids for v3a ablations.
...
It is intentionally not a new training implementation.  It trains only
long_H120 by default, uses the existing split manifest, and never reads test.
"""
...
from swing_bot.features.ablation import write_ablation_configs  # noqa: E402
...
DEFAULT_CONFIG_DIR = Path("configs/features/ablations/long_H120_v3a")
DEFAULT_FEATURE_OUTPUT_DIR = btcjpy_1m_processed_dir() / "feature_ablations" / "long_H120_v3a"
DEFAULT_MANIFEST_OUTPUT_DIR = outputs_dir() / "valid" / "features" / "ablations" / "long_H120_v3a"
...
def _find_configs(config_dir: Path, names: list[str]) -> list[Path]:
    if not config_dir.exists():
        write_ablation_configs(config_dir, force=False)
    if names:
        paths = []
        for name in names:
    ...
    if not paths:
        ...
        paths = sorted(config_dir.glob("entry_feature_set_long_H120_*.yaml"))
    if not paths:
        raise FileNotFoundError(f"no ablation configs found under {config_dir}")
    return paths
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build and train long_H120 v3a ablation entry grids.")
    ...
    parser.add_argument("--ablations", type=str, default=None, help="Comma-separated ablation names, e.g. v3a_full,v3a_minus_volume_price_pressure")
    ...
    parser.add_argument("--grid-prefix", type=str, default="long_h120_v3a_ablation")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        ...
        "notes": [
            ...
            "Compare outputs with scripts/compare_long_h120_v3a_ablation.py.",
        ],
