#!/usr/bin/env python3
"""Compare selected long_H120 entry candidate results.

The script reads existing entry_model_comparison.csv files. It does not train,
tune thresholds, read test data, or select a production setting.
"""
...
DEFAULT_OUTPUT = outputs_dir() / "valid" / "entry_grid" / "selected_long_H120_comparison.csv"
DEFAULT_GRID_PREFIX = "selected_long_H120"
DEFAULT_BASELINE = "long_H120_v0"
...
def _candidate_from_grid_id(grid_id: str, prefix: str) -> str:
...
def _find_grid_dirs(grid_root: Path, prefix: str, explicit: list[Path]) -> list[Path]:
    if explicit:
        return explicit
    dirs = sorted(p for p in grid_root.glob(prefix + "_long_H120*") if p.is_dir())
    if not dirs:
        dirs = sorted(p for p in grid_root.glob(prefix + "_*") if p.is_dir())
    if not dirs:
        raise FileNotFoundError(f"no selected grid dirs found under {grid_root} matching {prefix}_*")
    return dirs
...
def _read_one(grid_dir: Path, prefix: str) -> pd.DataFrame:
    ...
    df.insert(1, "candidate", _candidate_from_grid_id(grid_dir.name, prefix))
    return df
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare selected long_H120 entry candidates.")
    ...
    parser.add_argument("--grid-dir", type=Path, action="append", default=[])
...
def main(argv: list[str] | None = None) -> int:
    ...
    combined = pd.concat([_read_one(d, args.grid_prefix) for d in grid_dirs], ignore_index=True)
    ...
    print(json.dumps({
        ...
        "notes": [
            ...
            "long_H120_v0 is the primary candidate; long_H120_tail_v0 is a strict-threshold tail comparison.",
