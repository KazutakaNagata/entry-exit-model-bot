#!/usr/bin/env python3
"""Compare long_H120 v3a combined-ablation entry-grid results.
...
DEFAULT_OUTPUT = outputs_dir() / "valid" / "entry_grid" / "long_H120_v3a_combined_ablation_comparison.csv"
DEFAULT_GRID_PREFIX = "long_h120_v3a_combined_ablation"
DEFAULT_BASELINE = "v3a_full"
...
def _ablation_from_grid_id(grid_id: str, prefix: str) -> str:
...
def _read_one(grid_dir: Path, prefix: str) -> pd.DataFrame:
    ...
    df.insert(1, "ablation", _ablation_from_grid_id(grid_dir.name, prefix))
    return df
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare long_H120 v3a combined-ablation results.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps({
        ...
        "notes": [
            ...
            "Prefer variants that improve q95/q97/worst metrics, not q99 alone.",
